...
export class Crawler extends EventEmitter {
    ...
    private async _crawl(pageUrl: string, currentDepth: number = 0): Promise<void> {
        ...
        // --- Robots.txt Check ---
        const isCrawlable = await RobotsTxt.canCrawl(pageUrl);
        if (!isCrawlable) {
            console.log(`[${ts()}] [SKIP] Blocked by robots.txt: ${pageUrl}`);
            this.emit(EVENT_TYPES.PAGE_ERROR, { url: pageUrl, message: 'Blocked by robots.txt' });
            return;
        }
